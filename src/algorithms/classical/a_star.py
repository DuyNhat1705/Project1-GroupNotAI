...
class A_Star(BaseAlgorithm):
    ...
    def solve(self, problem, seed=None):
        ...
        if maze[goal[1]][goal[0]] == 1 or maze[start[1]][start[0]] == 1:
            return {"cost": 0, "path": [], "simualated matrix": [[]]}
        maze = problem.get('matrix')
        x, y = maze.shape
        ...
        return { "cost": best_g[tuple(goal)], 
                 "path": self.params["path"],
                 "simulated matrix": self.formatMatrixWithOptimalPath(maze)}
